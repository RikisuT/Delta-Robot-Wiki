...
import { Activity, ArrowRightLeft, Box, X, FileText, Sliders, Cpu, Database, Settings, ChevronDown, ChevronUp, Info, Palette } from 'lucide-react';
...
const systemData = {
  nodes: {
    'A':      { title: 'User / Script / GUI', type: 'External Interface', description: 'The entry point for triggering robot actions. Users edit task definitions, write G-code files, and invoke launch files to start either the simulation pipeline or the real-hardware pipeline.', actions_taken: ['Edits task JSON files (config/examples/)', 'Writes .gcode files for CNC-style paths', 'Launches delta_robot_spawn.launch.py (sim)', 'Launches delta_robot.launch.py (hardware)', 'Launches gcode_json_tools.launch.py (task runners)'] },
    'B_CONF': { title: 'Task & Parameters', type: 'Configuration', description: 'Contains the JSON sequence definition and node-specific parameters. JSON tasks define actions (move, home, wait, tilt, spin, suction) with coordinates in meters or mm. Example task JSON defines a sequence of move/wait/home actions with xyz coordinates and durations.', files: ['config/examples/example_task.json', 'config/examples/circle_test.gcode', 'config/delta_config.yaml', 'config/ros2_controllers.yaml'], parameters: ['json_units (meters|mm)', 'default_duration_s', 'motion_rate_hz', 'home_x_mm / home_y_mm / home_z_mm'] },
    ...
    'FOX':    { title: 'Foxglove Bridge', type: 'WebSocket Bridge', description: 'WebSocket server (port 8765) for Foxglove Studio remote visualization. Auto-launched by the simulation pipeline. Enables real-time topic inspection from any browser.', parameters: ['port (8765)', 'Auto-launched by delta_robot_spawn.launch.py'], actions_taken: ['WebSocket server on ws://localhost:8765', 'Foxglove Studio browser connection', 'Real-time topic/service introspection'] },
  ...
  edges: {
    'e1':  { title: 'Task Commands',       type: 'Service Calls', description: 'User triggers task execution via JSON files or direct service calls to the sequencer node.' },
    ...
    'e2':  { title: 'Task Loading',        type: 'File I/O', description: 'Task Sequencer reads JSON task definitions from config/examples/ directory at startup.' },
...
// ═══════════════════════════════════════════════════════════════
const nodesLayout = {
  'A':      { x: 480, y: 50,  color: '#059669', label: 'User / GUI', typeText: 'Interface' },
  'B_CONF': { x: 760, y: 50,  color: '#0f766e', label: 'Task Configs', typeText: 'JSON / G-code' },
  'K':      { x: 120, y: 200, color: '#475569', label: 'Launch & Config', typeText: 'System Files' },

  'B2':     { x: 340, y: 170, color: '#2563eb', label: 'G-code Parser', typeText: 'Python Node' },
  'B':      { x: 620, y: 170, color: '#2563eb', label: 'Task Sequencer', typeText: 'Python Node' },

  'IFACES': { x: 900, y: 290, color: '#6366f1', label: 'Interfaces Pkg', typeText: 'Msgs / Srvs' },
  'C':      { x: 480, y: 290, color: '#4f46e5', label: 'Motion Planner', typeText: 'C++ Core' },

  'M':      { x: 150, y: 440, color: '#b45309', label: 'Motor Control', typeText: 'Serial Driver' },
  'D':      { x: 430, y: 440, color: '#4f46e5', label: 'Kinematics', typeText: 'C++ Math' },
  'E':      { x: 710, y: 440, color: '#7c3aed', label: 'ROS2 Control', typeText: 'Controller' },
  'H':      { x: 990, y: 440, color: '#d97706', label: 'Gazebo Physics', typeText: 'Simulation' },

  'G':      { x: 710, y: 620, color: '#7c3aed', label: 'JS Broadcaster', typeText: 'Broadcaster' },
  'F':      { x: 990, y: 620, color: '#0891b2', label: 'ROS GZ Bridge', typeText: 'Middleware' },

  'P1':     { x: 150, y: 800, color: '#db2777', label: 'Servo Plotter', typeText: 'Matplotlib' },
  'P2':     { x: 430, y: 800, color: '#db2777', label: '3D EE Plotter', typeText: 'Matplotlib' },
  'I':      { x: 710, y: 800, color: '#2563eb', label: 'JS Bridge (Py)', typeText: 'Sim Filter' },
  'J':      { x: 990, y: 800, color: '#db2777', label: 'RViz2', typeText: 'Visualization' },
  'FOX':    { x: 1230, y: 800, color: '#0891b2', label: 'Foxglove', typeText: 'WebSocket' },
};

const edgesRaw = [
  { id: 'e1',  from: 'A', fs: 'bottom', fo: 15, to: 'B', ts: 'top', toOff: -15, label: 'Task Triggers' },
  { id: 'e12', from: 'A', fs: 'bottom', fo: -30, to: 'K', ts: 'top', toOff: 0, label: 'Launches', t: 0.3 },
  { id: 'e2',  from: 'B_CONF', fs: 'bottom', fo: -15, to: 'B', ts: 'top', toOff: 15, label: 'Reads JSON' },
  
  { id: 'e3',  from: 'B', fs: 'bottom', fo: 0, to: 'C', ts: 'top', toOff: 20, label: 'Motion Svc' },
  { id: 'e3b', from: 'B2', fs: 'bottom', fo: 0, to: 'C', ts: 'top', toOff: -20, label: 'G-code → Plan' },
  { id: 'e16', from: 'IFACES', fs: 'left', fo: 0, to: 'C', ts: 'right', toOff: 0, label: 'Msg/Srv Types' },
  { id: 'e13', from: 'K', fs: 'bottom', fo: 0, to: 'D', ts: 'top', toOff: -60, label: 'Config', t: 0.25 },

  { id: 'e4',  from: 'C', fs: 'bottom', fo: 0, to: 'D', ts: 'top', toOff: 0, label: 'IK/FK Math' },
  { id: 'e5b', from: 'C', fs: 'bottom', fo: -30, to: 'M', ts: 'top', toOff: 0, label: 'Traj → Motors' },
  { id: 'e5',  from: 'C', fs: 'bottom', fo: 30, to: 'E', ts: 'top', toOff: 0, label: 'Traj → Gazebo' },

  { id: 'e6',  from: 'E', fs: 'bottom', fo: 0, to: 'G', ts: 'top', toOff: 0, label: 'HW State' },
  { id: 'e7',  from: 'G', fs: 'bottom', fo: 0, to: 'I', ts: 'top', toOff: -20, label: '/joint_states' },
  { id: 'e9',  from: 'H', fs: 'bottom', fo: 0, to: 'F', ts: 'top', toOff: 0, label: 'Sim Topics' },
  { id: 'e10', from: 'F', fs: 'bottom', fo: 20, to: 'J', ts: 'top', toOff: 0, label: 'Bridge → RViz' },
  { id: 'e11', from: 'F', fs: 'bottom', fo: -20, to: 'I', ts: 'top', toOff: 20, label: 'Viz Data' },

  { id: 'e14', from: 'M', fs: 'bottom', fo: -20, to: 'P1', ts: 'top', toOff: 0, label: 'Servo Data' },
  { id: 'e15', from: 'D', fs: 'bottom', fo: 0, to: 'P2', ts: 'top', toOff: 0, label: 'EE Config' },

  { id: 'e8',  from: 'I', fs: 'right', fo: 0, to: 'D', ts: 'right', toOff: 0, label: 'Sim Feedback', custom: 'e8' },
  { id: 'e8b', from: 'M', fs: 'bottom', fo: 20, to: 'D', ts: 'bottom', toOff: -20, label: 'HW Feedback', custom: 'e8b' },
  { id: 'e17', from: 'D', fs: 'left', fo: 0, to: 'M', ts: 'right', toOff: 0, label: 'Joint Limits' }
];
...
function generateEdges() {
  return edgesRaw.map(e => {
    ...
    if (e.custom === 'e8') {
      c1x = x1 + 150; c1y = y1 - 50;
      c2x = x2 + 150; c2y = y2 + 50;
    }
    if (e.custom === 'e8b') {
      c1y = y1 + 80; c2y = y2 + 80;
    }
...
export default function App() {
  ...
  const [viewBox, setViewBox] = useState({ x: -100, y: -50, w: 1550, h: 1000 });
  ...
  return (
    <div className="h-screen w-full bg-slate-950 font-sans text-slate-200 overflow-hidden relative">
      ...
      <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
        ...
        <button onClick={() => setViewBox(vb => ({ ...vb, x: vb.x - vb.w*0.05, y: vb.y - vb.h*0.05, w: vb.w*1.1, h: vb.h*1.1 }))}
          className="bg-slate-800/90 hover:bg-slate-700 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold border border-slate-600 shadow-lg">−</button>
        <button onClick={() => setViewBox({ x: -100, y: -50, w: 1550, h: 1000 })}
