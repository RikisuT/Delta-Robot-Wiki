...
const systemData = {
  nodes: {
    ...
    'C':      { title: 'motion_planner (C++)', type: 'C++ ROS2 Node', description: 'The core trajectory generation engine. Receives high-level move commands, delegates IK/FK math to kinematics, and publishes motor commands on a separate execution thread. Supports 5 demo trajectories and custom point-list trajectories via PlayCustomTrajectory. Publishes to both simulation (JointTrajectory) and hardware (DeltaJoints) simultaneously. Live mode uses ROS topics for continuous command streaming.', topics_published: ['delta_motors/set_joints (DeltaJoints)', 'delta_motors/set_joint_vels (DeltaJointVels)', '/joint_trajectory_controller/joint_trajectory'], services_provided: ['play_demo_trajectory', 'move_to_point', 'move_to_configuration', 'motion_demo (start/stop auto-demo)', 'play_custom_trajectory'], services_called: ['delta_kinematics/delta_ik', 'delta_kinematics/delta_fk', 'delta_kinematics/convert_to_joint_trajectory', 'delta_kinematics/convert_to_joint_vel_trajectory'], parameters: ['traj_step_ms (default 10 ms)'], threads: ['TrajectoryExecutor — detached std::thread for non-blocking', 'Init Timer — deferred until kinematics online', 'Demo Timer — 32s auto-demo cycle'], files: ['src/motion_planner.cpp', 'include/motion_planner.hpp'], actions_taken: ['Live mode: topic stream via delta_motors/set_joints + delta_motors/set_joint_vels', 'Demo: circle — XY circle at Z=-180mm, R=40mm', 'Demo: pringle — saddle-shaped 3D path', 'Demo: axes — sequential X/Y/Z axis translations', 'Demo: up_down — 5 cycle vertical oscillation', 'Demo: scan — snake-scan from CSV file'] },
    ...
    'M':      { title: 'motor_control_node.py', type: 'Python ROS2 Node', description: 'Hardware serial driver and feedback concentrator over ESP32 bridge. Controls 5-DOF Waveshare servos (IDs 1-3 base, 4-5 end-effector) and serves as the primary feedback source for runtime state. Supports binary (0x7E SETN + XOR CRC) and text (SET/PING/SCAN) protocols. Tolerates missing motors, runs watchdog-protected feedback loop, and auto-detects /dev/ttyUSB* or /dev/ttyACM* at 500000 baud. Sensor feedback path is being consolidated to ESP32, reducing reliance on a separate delta_robot_sensors runtime node.', topics_subscribed: ['delta_motors/set_joints (DeltaJoints)', 'delta_motors/set_joint_vels (DeltaJointVels)'], topics_published: ['delta_motors/motor_position_feedback (DeltaJoints)', 'delta_motors/motor_velocity_feedback (DeltaJointVels)', '/servo/target (Float32MultiArray)', '/servo/actual (Float32MultiArray)'], services_provided: ['delta_motors/set_joint_limits'], parameters: ['device_name (auto-detect)', 'baudrate (500000)', 'bicep moving_speed / moving_acc', 'ee_moving_speed / ee_moving_acc', 'max_write_retries (3)', 'use_binary_bridge (true)', 'stream_feedback_period_ms (20)', 'read_fail_watchdog_limit (5)'], files: ['delta_robot/motor_control_node.py'], actions_taken: ['Binary protocol: 0x7E SOF + SETN cmd + XOR CRC', 'Text protocol: SET / PING / SCAN / TORQUE / LIST', 'Motor position: 0–4095 ticks (12-bit, 2048=center)', 'Conversion: θ·(4096/2π) + 2048 = motor ticks', 'Auto-detect /dev/ttyUSB* + /dev/ttyACM* at startup', 'Streamed feedback: FBP id=N pos=P speed=S', 'ESP32-centric feedback: motor + sensor state aggregation', '5-DOF: controls biceps + end-effector DOF simultaneously'] },
    ...
    'K':      { title: 'System Launch & Config', type: 'Launch / YAML Configs', description: 'Runtime orchestration and configuration for major_project_ws2. Simulation uses delta_robot_spawn.launch.py; hardware startup is typically done with ros2 run commands for kinematics, motion_planner, and motor_control_node.py as documented in README. Sensor runtime is intentionally simplified as ESP32 feedback integration takes over.', files: ['delta_robot_sim/launch/delta_robot_spawn.launch.py', 'delta_robot_gui/delta_robot_gui.launch.py', 'delta_robot/launch/gcode_json_tools.launch.py', 'delta_robot/config/ros2_controllers.yaml', 'delta_robot/config/delta_config.yaml', 'delta_robot_sim/config/ros_gz_bridge.yaml'], actions_taken: ['Sim pipeline: Gazebo + RViz + controllers + bridge + 3D plotter', 'HW runtime: kinematics + motion_planner + motor_control_node.py', 'Sensor role reduced: ESP32-based feedback path preferred', 'GUI runtime: PyQt control center via delta_robot_gui launch', 'Task pipeline: json_task_sequencer or gcode_parser'] },
  ...
  edges: {
    ...
    'e5b':    { title: 'Traj → Hardware',     type: 'Topic', description: 'Live mode topic stream: motor commands published as DeltaJoints/DeltaJointVels to motor_control_node.' },
    ...
    'e8b':    { title: 'HW Feedback',         type: 'Serial + Topics', description: 'ESP32-centric feedback path: motor_control ingests serial feedback and publishes runtime state to kinematics; dedicated sensor-node dependency is minimized.' },
    ...
    <div className="h-screen w-full bg-slate-950 font-sans text-slate-200 overflow-hidden relative flex flex-col">
      ...
      {!selectedId && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 bg-slate-900/90 backdrop-blur-xl px-5 py-2.5 rounded-xl border border-slate-700/50 shadow-xl pointer-events-none">
          ...
          <p className="text-[10px] text-cyan-300/90 mt-0.5 text-center">Live mode uses ROS topics: delta_motors/set_joints and delta_motors/set_joint_vels</p>
        </div>
      )}
